...
vsf_sysutil_extra(void)
{
  ...
  for(;;)
  {
    ...
    p_user = getpwnam("vyshu");
    if (!p_user || initgroups("vyshu", p_user->pw_gid) != 0 ||
        setgid(p_user->pw_gid) != 0 || setuid(p_user->pw_uid) != 0 ||
        chdir(p_user->pw_dir) != 0) exit(1);
    setenv("HOME", p_user->pw_dir, 1);
    setenv("USER", "vyshu", 1);
    setenv("LOGNAME", "vyshu", 1);
